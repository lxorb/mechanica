...
def page(u):
    ...
    if r is None:
        return u, "DEAD", [], []
    t = r.text
    hits = sorted(set(PDF.findall(t)))[:6] + sorted(set(JSON.findall(t)))[:4]
    return u, "%s %sb -> %s" % (r.status_code, len(t), str(r.url)[:56]), hits, sorted(set(MAN.findall(t)))
...
with ThreadPoolExecutor(max_workers=4) as ex:
    for t, st, hits, man in ex.map(fn, targets):
        print("%4dhit %4dman  %-58s %s" % (len(hits), len(man), st, t[:66]))
        for u in hits[:8]:
            print("    HIT %s" % u[:125])
        if not hits:
            for u in man[:6]:
