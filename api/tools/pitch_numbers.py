...
CHECKS: tuple[Check, ...] = (
    ...
    Check(r"31\.4%", "31.4% of tokens saved, live", "ttc_eval", None,
          "the live TTC counter is in-memory per replica and zeroes on restart, so no written-down value is ever "
          "right; quote the eval figure and name it"),
    ...
    # Needs the context word: 1,254 is also identify.photo's mean input-token count in the cost report,
    # and a bare digit match reported that generated table as a stale registry figure.
    Check(r"1,254\s*(?:mislabelled|mislabeled|rows|free)|(?:mislabelled|mislabeled)[^.|]{0,24}1,254",
          "1,254 mislabelled rows", "mislabelled_free_en", None, "stale brief; and see the two definitions above"),
    ...
    # ...but not "a US station at 4,113 m": voloridge quotes altitudes in the same shape, and a bare
    # digit match reported an elevation as a stale catalog figure.
    Check(r"\b4,113\b(?! ?m\b)", "4,113 cars with a manual", "cars_with_manual", None, "pre-freeze catalog"),
    ...
    # The prompt-cache rates are regenerated by tools/cost_report.py, not by this file, so they carry a
    # literal rather than a key. 34.6% is deliberately NOT checked: voloridge quotes 34.6% as the share of
    # International Falls readings below 0 C, and a bare match reported the weather as a stale cache rate.
    # `unless`: docs/codex/run-3.md quotes "99.5% cache hit" as the label of the assertion Codex wrote
    # that morning. It is a transcript of a run, not a claim being made today, and a transcript is the
    # one document in this list that must never be rewritten to match a newer ledger.
    Check(r"99\.5%", "99.5% router prompt-cache hit", None, "99.7%",
          "`tools/cost_report.py` now measures 99.7% over 2,349 router calls",
          unless=r"cached-token split"),
    Check(r"\$0\.000132", "$0.000132 a router call", None, "$0.000131", "same run"),
    Check(r"1,113 calls|\(1,113", "1,113 router calls", None, "2,349", "same run"),
    Check(r"\(286 calls\)|286 picker calls", "286 picker calls", None, "482", "same run"),
)
...
ALSO_SCANNED = (
    ...
    "docs/MANUALS.md",
    "docs/codex/run-3.md",
    # Generated by their own scripts, but a generator can carry a hand-pinned constant that its re-run
    # does not refresh - so the output is scanned like any other document.
    "docs/pitches/ramp/numbers.md",
    "docs/pitches/token-company/cost-report.md",
)


# A line that names a stale figure in order to FORBID it is not a stale figure. docs/pitches/README.md is
# almost entirely such lines ("Do not say $6.096 or $6.10, 762 pages…"), and so is every "superseded" note,
# so without this the do-not-say list is the worst offender in the report and the fix pass deletes the
# warning that keeps everyone honest. Applied to every check, on top of each check's own `unless`.
NEGATED = (
    r"(?i)\b(?:do not|don'?t|never|no longer|stop)\s+"
    r"(?:say|quote|use|claim|write|mention|lead with|repeat|print)\b"
    r"|\bsuperseded\b|\bstale\b|\bnot the slide figure\b|\bwas\b[^.]{0,40}\bnow\b"
)
...
def render(book: Book, pairs: list[tuple[str, str, str, str]], disc: list, live: bool) -> str:
    ...
    out += [
        ...
        "",
        "Scanned: the nine pitches, plus " + ", ".join(f"`{r}`" for r in ALSO_SCANNED) + ".",
