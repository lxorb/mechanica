...
`js/voice-deepgram.js` mints a key at `POST /voice/deepgram-token`, opens `wss://agent.deepgram.com/v1/agent/converse` with the `[scheme, key]` subprotocol (a browser socket has no headers), forwards `settings` unread, streams the mic as 24 kHz linear16 from an AudioWorklet, and plays the returned linear16 through an 80 ms jitter buffer. `show_page` is the one client-side function: it jumps the reader, the same path a `[p. N]` citation chip takes.
...
**Limits.** `PUBLIC_BASE` must be https or the endpoints are refused (503 here, not a dead socket). The Deepgram key needs `keys:write`, else `/voice/deepgram-token` 502s and VOICE says "Voice unavailable". English only. No mic, no toggle.
