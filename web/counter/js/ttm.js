...
/**
 * Released one painted frame later, not one task later. A screen that has been mounted has not
 * been *drawn* yet, and a task boundary comes before the next frame does: letting the index go
 * there put a 1.1 s build between the search field being in the DOM and the search field being
 * on the glass. rAF, then a task inside it, is "after the browser has painted".
 */
export function uiUp() {
  ...
  if (typeof requestAnimationFrame === "function") requestAnimationFrame(() => setTimeout(drain, 0));
