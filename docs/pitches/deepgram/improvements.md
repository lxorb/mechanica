...
| 8 | **Numbers normalised server-side** — return `"one hundred newton metres"` next to `"100 Nm"` in the tool result, so correct speech is data rather than an instruction the model may drop | **4** — measured over 8 live turns, every *figure* came out spelled, but 2 turns leaked a printed **grade** through raw (`SAE 15W/50`, `DOT 4 or DOT 5.1`), which aura-2 reads as letters. A prompt rule is not a guarantee | 1.5 h | not done |
